...
def game_loop(players: list) -> list:
    ...
    for round_index in range(1, 6):
        ...
        while True:  # Select cards until something breaks
            ...
            try_to_extend = (
                game.extend_tunnel()
            )  # Returns false if the round is over, the tunnel could not be extended
...
if __name__ == "__main__":
    players = [
        ...
        playerclasses.CarefulPlayer(0.5),
    ]
    ...
    for player in players:
        print()
        player_scores[player.name]["mean"] = numpy.mean(
            player_scores[player.name]["scores"]
        )
        print(f"{player.name} has a mean of {player_scores[player.name]['mean']}")

        player_scores[player.name]["median"] = numpy.median(
            player_scores[player.name]["scores"]
        )
        print(f"{player.name} has a median of {player_scores[player.name]['median']}")

        player_scores[player.name]["standard_deviation"] = numpy.std(
            player_scores[player.name]["scores"]
        )
        print(
            f"{player.name} has a standard deviation of {player_scores[player.name]['standard_deviation']}"
        )
