...
class BasePlayer:
    ...
    def _estimated_rubies_remaining(self, game_tunnel: list, guessed_num_players: int = 2) -> int:
        rubies_remaining = 0
...
class EstimatedRubiesPlayer(BasePlayer):
    ...
    def are_you_staying(self, game_state):
        ...
        if self._estimated_rubies_remaining(game_state["tunnel"], 2) >= self.target_rubies:
            return False
        return True
...
class CarefulPlayer(BasePlayer):
    ...
    def are_you_staying(self, game_state):
        ...
        if self.held_rubies == 0 and self._estimated_rubies_remaining(game_state["tunnel"], max([game_state["players_playing"]-1, 1])) <= 0:
            return True
        ...
        for trap_type in game_state["trap_types"]:
            ...
            if game_state["traps_discarded"].count(trap_type) < 2 and trap_type in game_state["traps_encountered"]:
                chance_of_death += (3-game_state["traps_discarded"].count(trap_type))/caverns_left_to_draw
